...
impl Slab {
    ...
    fn reserve(&mut self, value: usize) {
        self.slab.reserve(value);
    }
...
fn mutate(c: &mut Criterion) {
    let mut group = c.benchmark_group("mutate (with index)");
...
fn iterate(c: &mut Criterion) {
    let mut group = c.benchmark_group("iterate (with index)");
    for i in [10, 100, 1_000, 10_000].iter() {
        group.bench_with_input(BenchmarkId::new("beton", i), i, |b, i| {
            let setup = || {
